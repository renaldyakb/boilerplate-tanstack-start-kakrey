...
Database schema diatur menggunakan **Prisma ORM**.

### Structure

- Schema definisi menggunakan fitur **Prisma Schema Folder**.
- Schema dipecah per-domain di folder `prisma/schema/`.
- `base.prisma`: Konfigurasi generator dan datasource.
- `[domain].prisma`: Definisi model (misal `todo.prisma`).

### Menambahkan Tabel Baru

1. Buat file baru di `prisma/schema/[domain].prisma`:
...
2. Jalankan command untuk update database dan client:
...
- Mengupdate database schema.
- Men-generate ulang `@prisma/client` (`npx prisma generate` otomatis dijalankan).

### Menggunakan Database di tRPC

Gunakan `db` dari `@/server/db`:
